...
export default function ImageReorderPopup({ onClose, onSave, setOpenPopUp }) {
  ...
  const handleSave = async () => {
    ...
    formData.append("json_payload", localStorage.getItem("response"));

    console.log(formData);
    try {
      // Example POST request
      const res = await fetch(
        "https://ce4e1f142d44.ngrok-free.app/process_json_images",
        {
          method: "POST",
          body: formData,
        }
      ...
      localStorage.setItem("total_mark",data.result.total)
      console.log("✅ Uploaded successfully:", data);

      // Pass data back to parent if needed
      onSave && onSave(data);
      const classId = localStorage.getItem("selectedClassroomId");
      await updateStudentMarks(classId);
      // Close popup
      setOpenPopUp(null);
    } catch (err) {
  ...
  async function updateStudentMarks(classId) {
    try {
      // 1️⃣ Get total marks from local storage
      const storedData = localStorage.getItem("total_mark");
      if (!storedData) {
        console.error("No total_mark data found in localStorage");
        return;
      }

      const totalMarks = JSON.parse(storedData);

      // 2️⃣ Send it to backend for update
      const response = await fetch(`http://localhost:8000/api/classrooms/${classId}/updateMarks`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ totalMarks }),
      });
